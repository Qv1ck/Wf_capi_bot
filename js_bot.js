 ...
 */
bot.start((ctx) => {
    console.log(`✓ Команда /start от ${ctx.from.first_name} (ID: ${ctx.chat.id})`);
    const message = 
        `🤖 *Warf_bot*\n\n` +
        `Still sane, exile ? \n\n` +
        `*Доступные команды:*\n` +
        ...
        `• Вылазка и Ночная Волна\n\n`;
    
    ctx.replyWithMarkdown(message); // Отправляем с поддержкой Markdown (*жирный*, _курсив_)
});
